...
def main(arg_space, params_dict, seed):
    #CUDA for pytorch
    head = "max"
    ...
    if arg_space.interactionsAnalysis:
        ...
        if arg_space.load_motif_weights:
            
            if arg_space.methodType in ['SATORI','BOTH']:
               #"create_dataset/tf_pairs_60.txt"
            
                _,_,_,_ = run_interaction_evaluation(output_dir+"/",pairs_file, motif_weights=True)
                
            if arg_space.methodType in ['FIS','BOTH']:
                _,_,_,_ = run_interaction_evaluation_fis(output_dir + "/",pairs_file, motif_weights=True)
        else:
            if arg_space.methodType in ['SATORI','BOTH']:
                #pairs_file = "create_dataset/tf_pairs_80.txt"
                #pairs_file = "/s/jawar/i/nobackup/Saira/latest/satori_v2/data/ToyData/Ctfs/tf_pairs_data_1.txt"

                _,_,_,_ = run_interaction_evaluation(output_dir+"/", pairs_file)
            if arg_space.methodType in ['FIS','BOTH']:
                _,_,_,_ = run_interaction_evaluation_fis(output_dir + "/", pairs_file)
...
if __name__ == "__main__":
    ...
    entropy_vals = np.logspace(-5, -2, 6)
    print(entropy_vals)
    arg_space = parseArgs()
    params_dict = get_params_dict(arg_space.hparamfile)
    skip = entropy_vals[0]
    skip_dir = arg_space.directory + "/entropy_" + str(skip) + "/E" +str(1)
    root_dir = arg_space.directory
    #entropy_vals = [0.01]
    for ent in entropy_vals:
        params_dict["entropy_reg_value"] = ent
        print(ent)
        for seed in range(0,3):
            arg_space.directory = root_dir + "/entropy_" + str(ent) + "/E" +str(seed+1)           
            satori_path = arg_space.directory+"/Interactions_SATORI/"
            if os.path.exists(satori_path):
                shutil.rmtree(satori_path)
                print("REMOVED SATORI PATH")

            # if arg_space.directory == skip_dir:
            #     continue
            print(arg_space.directory)
            #arg_space.mode = "train"
            main(arg_space, params_dict, seed)
