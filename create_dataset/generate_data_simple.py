...
def motifs2embed(pos = True, copy_pair = False):
    '''
...
def motifs2embed_simple(pos = True, copy_pair = False):
    '''
    This method returns the ids of motifs to be embedded in positive and negative sequences. 
    Number of pairs : 80
    Number of unique TFs : 93
    ...
    if pos: #Positive sequence
        #randomly pick number of pairs
        no_pairs = 2 #pair_choices[np.random.choice(2, p=pair_prob)]
        ...
        no_tfs = 3  #randint(1,4)
        total_unique = 8
        values = random.sample(range(total_pairs), no_tfs)
        print(values)

        for i in range(no_tfs):
            #randomly pick motif to embed from 93 unique motifs
            value = randint(0, 1)
            idx = (values[i])*2 + value
            print(idx)
            pairs.append(idx)
    return pairs
...
def generateData(no_of_positive, no_of_negative, seq_len, pwms, outfile_name = "seqs93_tf_2_a"):
    ...
    for i in range(no_of_negative):
        ...
        idxs = motifs2embed_simple(pos=False)
        #get motif sequence everytime using letter probability matrix
...
if __name__ == "__main__":
    ...
    success = generateData(num_of_seq,num_of_seq, seq_len,pwms, "simple_data") 
