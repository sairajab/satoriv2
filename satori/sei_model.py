...
class BSplineTransformation(nn.Module):
    ...
    def forward(self, input):
        if self._spline_tr is None:
            ...
            self._spline_tr = spline_factory(
                spatial_dim, self._df, log=self._log)
            if self._scaled:
...
class BSplineConv1D(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, kernel_size, degrees_of_freedom, stride=1,
        ...
        self.spline = nn.Conv1d(1, degrees_of_freedom, kernel_size, stride, padding, dilation,
                                bias=False)
        self.spline.weight = spline_factory(
            kernel_size, self._df, log=log).view(self._df, 1, kernel_size)
        if scaled:
    ...
    def forward(self, input):
        ...
        spline_out = self.spline(input.view(
            batch_size * n_channels, 1, length))
        conv1d_out = self.conv1d(spline_out.view(
            batch_size, n_channels * self._df,  length))
        return conv1d_out


class Sei_light(nn.Module):
    def __init__(self, input_size, sequence_length=4096, big = True, bspline = False, n_genomic_features=21907):
        super(Sei_light, self).__init__()
        ...
        if big:
            ...
            self.dconv2 = nn.Sequential(
                nn.Dropout(p=0.4),
                nn.Conv1d(640, 640, kernel_size=5, dilation=4, padding=8),
                nn.ReLU(inplace=True))
            self.output_dim = 640
    ...
    def forward(self, x):
        ...
        if self.big:
            ...
            dconv_out1 = self.dconv1(out2 + lout2)
            cat_out1 = out2 + dconv_out1
            
        else:
            
            dconv_out1 = self.dconv1(out1 + lout1)
        ...
        if self.bspline:
            ...
            reshape_out = spline_out.view(
                spline_out.size(0), self.output_dim * self._spline_df)
...
class Sei(nn.Module):
    def __init__(self, sequence_length=600, n_genomic_features=2): #164
        ...
        self.dconv3 = nn.Sequential(
            nn.Dropout(p=0.20),
            nn.Conv1d(640, 640, kernel_size=5, dilation=8, padding=16),
            nn.ReLU(inplace=True))
        self.dconv4 = nn.Sequential(
            nn.Dropout(p=0.10),
            nn.Conv1d(640, 640, kernel_size=5, dilation=16, padding=32),
            nn.ReLU(inplace=True))
